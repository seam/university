...
xw.navigationManagers = [];

xw.registerNavigationManager = function(m) {
  xw.navigationManagers.push(m);
};
...
xw.Action.prototype.invoke = function(locals) {
  if (xw.Sys.isDefined(this.script)) {
    ...
    if (xw.Sys.isDefined(locals)) {
      for (var local in locals) {
        __registered[local] = locals[local];
        __script += "var " + local + " = __registered[\"" + local + "\"];";
      }
    }
...
xw.Widget.prototype.toString = function() {
  return this._className + "[" + this.id + "]";
};
...
xw.NavigationManager = function() {
  xw.NonVisual(this);
  xw.registerNavigationManager(this);

};

xw.initNavigation = function() {
  // Initialize
  alert("onload");
};

//xw.Sys.chainEvent(window, "load", xw.initNavigation);

xw.NavigationManager.prototype = new xw.NonVisual();
